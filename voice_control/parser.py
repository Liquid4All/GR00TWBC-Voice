...
Design notes
------------
* Everything is lower-cased, punctuation-stripped and whitespace-collapsed.
* Spoken numbers ("one point five") are converted to digits before extraction.
* Detection runs in a fixed precedence order so that, e.g., an "emergency stop"
  is never mis-read as navigation.
* When the command is ambiguous or unsupported we return a ``clarify`` tool call
  with low confidence so the runtime never moves on a guess.
"""
...
def normalize_text(text: str) -> str:
    """Lower-case, strip punctuation, collapse whitespace, expand numbers."""
...
def _words_to_numbers(text: str) -> str:
    """Convert spoken number words (incl. decimals via 'point') to digits."""
...
def _consume_number(tokens: List[str], start: int) -> Tuple[float, int]:
    """Parse a (possibly compound, possibly decimal) spoken number."""
...
class DeterministicParser:
    """Stateless deterministic parser. Optionally aware of the current mode."""
...
def parse_text(text: str, confidence_threshold: float = 0.75,
    ...
    """Convenience wrapper around :class:`DeterministicParser`."""
...
def parse_plan(text: str, confidence_threshold: float = 0.75,
    ...
    """Convenience wrapper that parses a compound utterance into a plan."""
